...
- (BOOL)hasCacheForKey:(NSString *)key {
    return [[NSFileManager defaultManager] fileExistsAtPath:[self pathForKey:[key aln_MD5String]]];
}
    ...
    dispatch_async(self.cacheQueue, ^{
        ...
        if (maxCacheAge<=0) {
            isTooOld=YES;
             ...
             diskResult=[AVPersistenceUtils getJSONFromPath:[self pathForKey:[key aln_MD5String]]];
            if (diskResult && maxCacheAge > 0) {
                NSDate *lastModified = [AVPersistenceUtils lastModified:[self pathForKey:[key aln_MD5String]]];
...
- (void)saveJSON:(id)JSON forKey:(NSString *)key {
    dispatch_async(self.cacheQueue, ^{
        [AVPersistenceUtils saveJSON:JSON toPath:[self pathForKey:[key aln_MD5String]]];
    });
}
...
- (void)clearCacheForKey:(NSString *)key {
    dispatch_sync(self.cacheQueue, ^{
        [[NSFileManager defaultManager] removeItemAtPath:[self pathForKey:[key aln_MD5String]] error:NULL];
    });
}
